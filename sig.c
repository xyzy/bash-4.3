...
set_signal_handler (sig, handler)
     ...
     SigHandler *handler;
{
  //printf("set_signal_handler  ------>  Start\n");
  struct sigaction act, oact;
  ...
  {
    //printf("if (sig == SIGTERM && handler == sigterm_sighandler)\n");
    act.sa_flags |= SA_RESTART;		/* XXX */
  }
  ...
  {
    //printf("if (sigaction (sig, &act, &oact) == 0)\n");
    return (oact.sa_handler);
  }
  else
  {
    //printf("return (SIG_DFL);\n");
    return (SIG_DFL);
  }
  //printf("set_signal_handler  ------>  Exit\n");
}
#endif /* HAVE_POSIX_SIGNALS */
