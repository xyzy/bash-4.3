...
send_pwd_to_eterm ()
{
  //printf("send_pwd_to_eterm  ------>  Start\n");
  char *pwd, *f;
...
/* Call the YACC-generated parser and return the status of the parse.
  ...
  /* Allow the execution of a random command just before the printing
     ...
     {
      //printf("interactive && bash_input.type != st_string\n");
      command_to_execute = get_string_value ("PROMPT_COMMAND");
      //printf("command_to_execute:%s\n", command_to_execute);
      if (command_to_execute)
      {
        //printf("command_to_execute\n");
        execute_variable_command (command_to_execute, "PROMPT_COMMAND");
      }

      if (running_under_emacs == 2)
      {
        //printf("running_under_emacs == 2\n");
  ...
  /* Only do timeouts if interactive. */
    ...
    if (interactive)
    {
      ...
      tmout_var = find_variable ("TMOUT");
        ...
        char *bashdir, *bashdirc=(char *)malloc(2048), *buffer[1024];
        bashdir = "/.bashdir";
      ...
      {
        //printf("if (tmout_var && var_isset (tmout_var))\n");
        tmout_len = atoi (value_cell (tmout_var));
        //printf("tmout_len:%s\n", tmout_len);
        if (tmout_len > 0)
        {
          //printf("if (tmout_len > 0)\n");
    ...
    current_command_line_count = 0;
    //printf("result = parse_command ()  ------>  Start\n");
    result = parse_command ();
    //printf("result = parse_command ()  ------>  Exit\n");

    if (interactive && tmout_var && (tmout_len > 0))
    {
      //printf("if (interactive && tmout_var && (tmout_len > 0))\n");
      alarm(0);
      set_signal_handler (SIGALRM, old_alrm);
    }

    //printf("read_command  ------>  Exit\n");
    return (result);
  }
