...
/* Execute the command passed in COMMAND, perhaps doing it asynchronously.
   ...
   {
    //printf("execute_command_internal ======>  Start\n");
    ...
    {
      //printf("command == 0 || read_but_dont_execute\n");
...
switch (command->type)
{
  case cm_simple:
  {
   ...
   save_line_number = line_number;
    ...
    {
      //printf("if (ignore_return && command->value.Simple)\n");
     command->value.Simple->flags |= CMD_IGNORE_RETURN;
    }
   if (command->flags & CMD_STDIN_REDIR)
   {
    //printf("if (command->flags & CMD_STDIN_REDIR)\n");
   ...
   (void) alloca (0);
    ...
    {
      //printf("if (already_making_children && pipe_out == NO_PIPE)\n");
     stop_pipeline (asynchronous, (COMMAND *)NULL);

     if (asynchronous)
     {
      //printf("if (asynchronous)\n");
     ...
     {
      //printf("if (last_made_pid != last_asynchronous_pid)\n");
#endif
    ...
    {
      //printf("if (was_error_trap && ignore_return == 0 && invert == 0 && pipe_in == NO_PIPE && pipe_out == NO_PIPE && exec_result != EXECUTION_SUCCESS)\n");
     last_command_exit_value = exec_result;
     line_number = line_number_for_err_trap;
     run_error_trap ();
     line_number = save_line_number;
   ...
   {
    //printf("if (ignore_return == 0 && invert == 0 && ((posixly_correct && interactive == 0 && special_builtin_failed) || (exit_immediately_on_error && pipe_in == NO_PIPE && pipe_out == NO_PIPE && exec_result != EXECUTION_SUCCESS)))\n");
     last_command_exit_value = exec_result;
     run_pending_traps ();
     jump_to_top_level (ERREXIT);
   }
...
/* Execute a command that's supposed to be in a subshell.  This must be
   ...
   struct fd_bitmap *fds_to_close;
   {
    //printf("execute_in_subshell  ------>  Start\n");
...
struct fd_bitmap *fds_to_close;
{
  //printf("execute_connection  ------>  Start\n");
  COMMAND *tc, *second;
...
FOR_COM *for_command;
{
  ...
  //printf("execute_for_command  ------>  Start\n");
...
{
  //printf("execute_arith_for_command  ------>  Start\n");
  intmax_t expresult;
...
/* Execute a SELECT command.  The syntax is:
   ...
   {
    //printf("execute_select_command  ------>  Start\n");
...
/* Execute a CASE command.  The syntax is: CASE word_desc IN pattern_list ESAC.
   ...
   {
    //printf("execute_case_command  ------>  Start\n");
...
/* The WHILE command.  Syntax: WHILE test DO action; DONE.
   ...
   {
    //printf("execute_while_command  ------>  Start\n");
...
/* UNTIL is just like WHILE except that the test result is negated. */
  ...
  WHILE_COM *while_command;
  {
    //printf("execute_until_command  ------>  Start\n");
...
/* The body for both while and until.  The only difference between the
   ...
   int type;
   {
    //printf("execute_while_or_until  ------>  Start\n");
...
/* IF test THEN command [ELSE command].
   ...
   IF_COM *if_command;
   {
    //printf("execute_if_command  ------>  Start\n");
...
{
  //printf("execute_arith_command  ------>  Start\n");
...
COND_COM *cond;
{
  //printf("execute_cond_node  ------>  Start\n");
...
{
  //printf("execute_cond_command  ------>  Start\n");
  int retval, save_line_number;
...
/* Execute a null command.  Fork a subshell if the command uses pipes or is
   ...
   {
    //printf("execute_null_command  ------>  Start\n");
    int r;
    int forcefork;
    REDIRECT *rd;
...
/* The meaty part of all the executions.  We have to start hacking the
   ...
   struct fd_bitmap *fds_to_close;
   {
    //printf("execute_simple_command  ------> Start\n");
...
int flags, subshell;
{
  ...
  /* The eval builtin calls parse_and_execute, which does not know about
     ...
     {
      //printf("subshell == 0 && (flags & CMD_IGNORE_RETURN) && (builtin == eval_builtin || builtin == command_builtin || builtin == source_builtin)\n");
...
WORD_LIST *words;
{
  //printf("execute_shell_function  ------>  Start\n");
  int ret;
  struct fd_bitmap *bitmap;
...
/* Execute a shell builtin or function in a subshell environment.  This
   ...
   int flags;
   {
    //printf("execute_subshell_builtin_or_function  ------>  Start\n");
...
/* Execute a builtin or function in the current shell context.  If BUILTIN
   ...
   int flags;
   {
    //printf("execute_builtin_or_function  ------>  Start\n");
    int result;
...
#  define NOTFOUND_HOOK "command_not_found_handle"
   ...
   int cmdflags;
   {
    ...
    //printf("execute_disk_command  ------>  Start\n");
...
/* If the operating system on which we're running does not handle
   ...
   char **args, **env;
   {
    //printf("execute_shell_script  ------>  Start\n");
...
{
  //printf("execute_intern_function  ------>  Start\n");
  SHELL_VAR *var;
